...
public class DingoOldClient extends ClientBase {
    ...
    */
    public DingoOldClient(String configPath, String tableName) throws Exception {
        this(configPath, tableName, 1990);
    }


    public DingoOldClient(String coordinatorExchangeSvrList,String tableName, Integer retryTime) {
        super(coordinatorExchangeSvrList);
        this.tableName = tableName;
        this.metaClient = new MetaClient(coordinatorExchangeSvrList);
        try {
            metaClient.init(null);
        } catch (Exception e) {
            log.error("Failed to initialize connection: {}", coordinatorExchangeSvrList, e);
            e.printStackTrace();
        }
