...
public class StoreOperationUtils {
    ...
    public ResultForClient doOperation(StoreOperationType type,
        ...
        RouteTable routeTable = getAndRefreshRouteTable(tableName, true);
        if (routeTable == null) {
        ...
        do {
            try {
                ...
                Map<String, ContextForStore> keys2Executor = groupKeysByExecutor(routeTable, type, tableName, context4Store);
                ...
                for (Map.Entry<String, ContextForStore> entry : keys2Executor.entrySet()) {
                    ...
                    System.out.println("Huzx=====> Loop: "
                        + index++ + ", Cnt:"
                        + entry.getValue().getKeyListInBytes().size()
                        + ", RecordSize: "
                        + entry.getValue().getRecordList().size());
                ...
                */
                ...
                if (result4Client.getStatus()) {
                    log.info("Execute operation:{} on table:{} cnt:{} success", type, tableName, keyValueList.size());
                    // System.out.println("Execute operation:" + type + " on table:" + tableName + " cnt:" + keyValueList.size());
                }
            } catch (Exception ex) {
    ...
    private Map<String, ContextForStore> groupKeysByExecutor(
        ...
        for (int index = 0; index < wholeContext.getKeyListInBytes().size(); index++) {
            ...
            String leaderAddress = getLeaderByStartKey(routeTable, keyInBytes);
    ...
    private synchronized String getLeaderByStartKey(final RouteTable routeTable,
                                                    byte[] keyInBytes) {
